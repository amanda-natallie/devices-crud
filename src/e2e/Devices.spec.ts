...
test('As an User, I can see the devices list and edit a device', async ({ page }) => {
  ...
  await expect(page.getByTestId('app-wrapper')).toContainText('DESKTOP-0VCBIFF');
  await expect(page.getByTestId('app-wrapper')).toContainText('Windows workstation - 128 GB');
